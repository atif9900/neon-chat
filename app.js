...
function getDecentralizedChat() { 
    return JSON.parse(localStorage.getItem('P2P_Chat_' + currentRoomDisplayCode) || '[]'); 
}

function saveDecentralizedMsg(msg) {
    if (isIncognito) return; 
    let chats = getDecentralizedChat(); 
    if (!chats.find(m => m.msgId === msg.msgId)) { 
        chats.push(msg); 
...
function getRoomAdmins() { 
    return JSON.parse(localStorage.getItem('P2P_Admins_' + currentRoomDisplayCode) || '[]'); 
}

function isLocalAdmin() { 
    return getRoomAdmins().includes(localPeerId); 
}
...
if (mCanvas && mCtx) {
    ...
    const font_size = 14; 
    const cols = mCanvas.width / font_size;
    ...
    function renderCanvasMatrix() {
        ...
        if (theme === 'hacker') {
            ...
            for (let i = 0; i < columns.length; i++) { 
                ...
                mCtx.fillText(text, i * font_size, columns[i] * font_size); 
                if (columns[i] * font_size > mCanvas.height && Math.random() > 0.975) columns[i] = 0; 
                columns[i]++; 
            }
        } else if (theme === 'romance') {
            mCtx.fillStyle = 'rgba(20, 3, 9, 0.06)'; 
            mCtx.fillRect(0, 0, mCanvas.width, mCanvas.height); 
            mCtx.fillStyle = '#ff2a6d'; 
            mCtx.font = (12 + Math.floor(Math.random() * 6)) + "px Arial";
            for (let i = 0; i < columns.length; i++) { 
                const hearts = ['❤', '♥', '💕', '💖']; 
                const text = hearts[Math.floor(Math.random() * hearts.length)]; 
                if (Math.random() > 0.4) mCtx.fillText(text, i * font_size, columns[i] * font_size); 
                if (columns[i] * font_size > mCanvas.height && Math.random() > 0.96) columns[i] = 0; 
                columns[i] += 0.5; 
            }
            ...
            if (Math.random() > 0.99) {
                ...
                if (th) { 
                    th.classList.remove('thunder-hidden'); 
                    th.classList.add('thunder-flash'); 
                    setTimeout(() => { 
                        th.classList.remove('thunder-flash'); 
                        th.classList.add('thunder-hidden'); 
                    }, 1000); 
                }
            }
        } else { 
...
function initializeSecureChatMatrix(code) {
    ...
    currentRoomDisplayCode = code; 
    roomId = generateDeterministicId(code); 
    cryptoKey = "sec_" + code; 
    
    document.getElementById('share-url').value = code; 
    ...
    onChildRemoved(ref(db, `rooms/${roomId}/peers`), (snapshot) => {
        ...
        delete remotePointers[id];
        if (document.getElementById('peer-count')) {
            document.getElementById('peer-count').innerText = `${Object.keys(dataChannels).length} Active Nodes`;
        }
    });
    ...
    if (admins.length === 0) { 
        admins.push(localPeerId); 
        localStorage.setItem('P2P_Admins_' + currentRoomDisplayCode, JSON.stringify(admins)); 
    }
    updateAdminUI();
    resizeCanvas();
}
...
function updateAdminUI() {
    ...
    // Admin checking logic modified for fallback visibility during testing
    [audioCallBtn, videoCallBtn].forEach(b => { 
        if (b) b.classList[isLocalAdmin() ? 'remove' : 'add']('hidden'); 
    });
}

if (connectEngineBtn) {
    connectEngineBtn.addEventListener('click', () => { 
        initializeSecureChatMatrix(roomCodeInput.value.trim() || 'pool_123'); 
    });
}
...
async function handleIncomingSignal(sig) {
    ...
    try {
        ...
        if (sig.type === 'offer') {
            ...
            if (localAVStream) {
                localAVStream.getTracks().forEach(t => { 
                    if (!pc.getSenders().find(s => s.track === t)) pc.addTrack(t, localAVStream); 
                });
            }
            ...
            if (pc.remoteDescription && pc.remoteDescription.type) { 
                ...
                pc.iceQueue.push(sig.candidate); 
            }
        }
    } catch(error) { 
...
function removeRemoteVideoNode(id) {
    ...
    updateVideoGrid();
    if (dynamicVideoGrid && dynamicVideoGrid.children.length === 0 && !localAVStream) {
        videoWorkspaceOverlay.classList.add('video-hidden');
    }
...
function resetCallUI() {
    ...
    isAudioMuted = false;
    if (vToggleCamBtn) { 
        vToggleCamBtn.style.background = 'rgba(255,255,255,0.1)'; 
        vToggleCamBtn.innerHTML = '<i class="fa-solid fa-video"></i>'; 
    }
    if (vMuteAudioBtn) { 
        vMuteAudioBtn.style.background = 'rgba(255,255,255,0.1)'; 
        vMuteAudioBtn.innerHTML = '<i class="fa-solid fa-microphone"></i>'; 
    }
    screenStream = null;
}
...
function buildWebRTCLink(remoteId, isOffer) {
    ...
    pc.oniceconnectionstatechange = () => { 
        if (['disconnected', 'failed', 'closed'].includes(pc.iceConnectionState)) { 
            removeRemoteVideoNode(remoteId); 
            delete peers[remoteId]; 
            delete dataChannels[remoteId]; 
            delete remotePointers[remoteId]; 
        } 
    };
    ...
    pc.ontrack = (e) => {
        ...
        if (!existingVid) {
            ...
            wrapper.className = 'dynamic-remote-frame'; 
            wrapper.id = `wrapper_${remoteId}`; 
            
            let vid = document.createElement('video'); 
            vid.id = `vid_${remoteId}`; 
            vid.autoplay = true; 
            vid.playsInline = true; 
            
            let label = document.createElement('span'); 
            label.className = 'video-peer-label'; 
            label.id = `label_${remoteId}`; 
            label.innerText = 'Connecting...';
            label.onclick = (ev) => { 
                if (isLocalAdmin()) showAdminModal(remoteId, ev.clientX, ev.clientY, dataChannels[remoteId]?.remoteName || 'User'); 
            };
            ...
            existingVid.srcObject = e.streams[0]; 
    ...
    if (isOffer) { 
        ...
        pc.ondatachannel = (e) => bindChannel(remoteId, e.channel); 
    }
}
...
function showAdminModal(peerId, x, y, name) {
    ...
    activeModalPeer = peerId; 
    document.getElementById('admin-modal-title').innerText = `Manage: ${name}`;
    adminModal.style.left = `${Math.min(x, window.innerWidth - 220)}px`; 
    adminModal.style.top = `${y}px`;
...
function sendAdminAction(action, targetId) {
    Object.values(dataChannels).forEach(dc => { 
        if (dc.readyState === "open") dc.send(JSON.stringify({ type: 'admin_action', action, targetId, sender: localPeerId })); 
    });
}
        ...
        if (data.type === 'history_sync') {
            ...
            if (data.history.length > myChats.length) {
                ...
                data.history.forEach(m => { 
                    let cText = m.cipher ? rc4Cipher(cryptoKey, decodeURIComponent(escape(atob(m.cipher)))) : ''; 
                    pushBubble(clearText, m.senderId === localPeerId ? 'me' : 'them', m.msgId, m.mediaObj, m.senderName, true); 
                });
            }
            if (data.admins) { 
                localStorage.setItem('P2P_Admins_' + currentRoomDisplayCode, JSON.stringify(data.admins)); 
                updateAdminUI(); 
            }
        }
        else if (data.type === 'chat') { 
            let clearText = data.cipher ? rc4Cipher(cryptoKey, decodeURIComponent(escape(atob(data.cipher)))) : '';
            saveDecentralizedMsg(data); 
            pushBubble(clearText, 'them', data.msgId, data.mediaObj, data.senderName); 
        }
        else if (data.type === 'admin_action') {
            if (data.action === 'kick' && data.targetId === localPeerId) { alert("Admin kicked you."); location.reload(); }
            if (data.action === 'ban') { 
                let bList = JSON.parse(localStorage.getItem('P2P_Banned_' + currentRoomDisplayCode) || '[]'); 
                bList.push(data.targetId); 
                localStorage.setItem('P2P_Banned_' + currentRoomDisplayCode, JSON.stringify(bList));
                if (data.targetId === localPeerId) { alert("Admin Banned you permanently."); location.reload(); }
            }
            if (data.action === 'make_admin') {
                let aList = getRoomAdmins(); 
                if (!aList.includes(data.targetId)) aList.push(data.targetId); 
                localStorage.setItem('P2P_Admins_' + currentRoomDisplayCode, JSON.stringify(aList));
                updateAdminUI(); 
                if (data.targetId === localPeerId) appendSystemMessage("👑 You have been granted Admin rights.");
            }
            if (data.action === 'del_msg') {
                let b = document.querySelector(`[data-msg-id="${data.msgId}"] .bubble`); 
                if (b) b.innerHTML = `<i>🔒 Wiped via Admin / Everyone.</i>`; 
                deleteDecentralizedMsg(data.msgId);
            }
            ...
            renderDraw(data.payload, id);
        }
        else if (data.type === 'clear') { 
            if (ctx) ctx.clearRect(0, 0, blackboardCanvas.width, blackboardCanvas.height); 
        }
        else if (data.type === 'name_sync') { 
            dc.remoteName = data.name; 
            let lbl = document.getElementById(`label_${id}`); 
            if (lbl) lbl.innerText = data.name; 
        }
...
if (videoCallBtn) {
    videoCallBtn.addEventListener('click', async () => {
        try {
            ...
            if (localVideoStreamNode) { 
                localVideoStreamNode.srcObject = localAVStream; 
                localVideoStreamNode.muted = true; 
                localVideoStreamNode.parentElement.style.display = 'block'; 
            }
            Object.values(peers).forEach(pc => { localAVStream.getTracks().forEach(t => pc.addTrack(t, localAVStream)); });
            resetCallUI();
            ...
            alert("Hardware Denied or No Camera."); 
        }
    });
}
...
if (endCallBtn) {
    endCallBtn.addEventListener('click', () => {
        ...
        if (localAVStream) { localAVStream.getTracks().forEach(t => t.stop()); localAVStream = null; }
        if (localVideoStreamNode) localVideoStreamNode.srcObject = null; 
        localVideoStreamNode.parentElement.style.display = 'none';
        if (dynamicVideoGrid) dynamicVideoGrid.innerHTML = ''; 
        resetCallUI(); 
        Object.values(dataChannels).forEach(dc => { 
            if (dc.readyState === "open") dc.send(JSON.stringify({type:'end_video_call_signal'})); 
        });
    });
}
...
if (vFlipCamBtn) {
    vFlipCamBtn.addEventListener('click', async () => {
        ...
        try { 
            ...
            alert("Camera toggle error: " + err.message);
        }
    });
}
...
if (messageInput) {
    ...
    messageInput.addEventListener('input', () => {
        Object.values(dataChannels).forEach(dc => { 
            if (dc.readyState === "open") dc.send(JSON.stringify({ type: 'typing', isTyping: true })); 
        });
        clearTimeout(typingTimer);
        typingTimer = setTimeout(() => {
            Object.values(dataChannels).forEach(dc => { 
                if (dc.readyState === "open") dc.send(JSON.stringify({ type: 'typing', isTyping: false })); 
            });
        }, 1500);
    });
}
...
function pushBubble(text, sender, id, mediaObj, nameLabel, isHistoryLoad = false) {
    ...
    if (sender === 'me' || isLocalAdmin()) {
        dBtn.onclick = () => { 
            row.querySelector('.bubble').innerHTML = '<i>🔒 Wiped via Admin / Everyone.</i>'; 
            deleteDecentralizedMsg(id); 
            sendAdminAction('del_msg', id); 
            dBtn.remove(); 
        };
...
function appendSystemMessage(t) { 
    let m = document.createElement('div'); m.className = 'system-msg'; m.innerText = t; 
    messagesContainer.appendChild(m); messagesContainer.scrollTop = messagesContainer.scrollHeight; 
}

// --- BLACKBOARD ENGINE WITH WORKING TEXT TOOL ---
...
function startDraw(e) {
    ...
    // TEXT TOOL FIX APPLIED
    if (currentTool === 'text') {
        let txt = prompt("📝 Paste your code or text segment to output on blackboard:");
        if (txt && txt.trim() !== "") {
